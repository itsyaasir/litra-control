...
pub async fn set_device_temperature(
    ...
    if kelvin < MIN_TEMPERATURE || kelvin > MAX_TEMPERATURE {
        return Err(AppError {
            message: format!(
                "Temperature {} K is out of range. Supported range: {}-{} K",
                kelvin, MIN_TEMPERATURE, MAX_TEMPERATURE
            ),
    ...
    if kelvin % TEMPERATURE_STEP != 0 {
        return Err(AppError {
            message: format!(
                "Temperature {} K is invalid. Must be a multiple of {} K",
    ...
    handle.set_temperature_in_kelvin(kelvin).map_err(|e| AppError {
...
pub async fn increase_device_temperature(
    ...
    if kelvin_increase % TEMPERATURE_STEP != 0 {
        return Err(AppError {
            message: format!(
                "Temperature increase {} K is invalid. Must be a multiple of {} K",
                kelvin_increase, TEMPERATURE_STEP
            ),
            error_type: "TemperatureIncrementError".to_string(),
        });
    }
    ...
    if new_temperature > max_temperature {
        return Err(AppError {
            message: format!(
                "Temperature increase would exceed maximum. Current: {} K, Increase: {} K, Max: {} K",
                current_kelvin, kelvin_increase, max_temperature
            ),
            error_type: "TemperatureRangeError".to_string(),
        });
    }
    
    handle.set_temperature_in_kelvin(new_temperature).map_err(|e| AppError {
...
pub async fn decrease_device_temperature(
    ...
    if kelvin_decrease % TEMPERATURE_STEP != 0 {
        return Err(AppError {
            message: format!(
                "Temperature decrease {} K is invalid. Must be a multiple of {} K",
                kelvin_decrease, TEMPERATURE_STEP
            ),
            error_type: "TemperatureIncrementError".to_string(),
        });
    }
    ...
    if new_temperature < min_temperature {
        return Err(AppError {
            message: format!(
                "Temperature decrease would fall below minimum. Current: {} K, Decrease: {} K, Min: {} K",
                current_kelvin, kelvin_decrease, min_temperature
            ),
            error_type: "TemperatureRangeError".to_string(),
        });
    }
...
pub async fn validate_temperature(kelvin: u16) -> Result<bool, AppError> {
    Ok(kelvin >= MIN_TEMPERATURE 
        && kelvin <= MAX_TEMPERATURE 
        && kelvin % TEMPERATURE_STEP == 0)
}
