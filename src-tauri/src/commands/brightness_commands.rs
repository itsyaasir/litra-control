...
pub async fn set_device_brightness(
    ...
    if lumens < min_brightness || lumens > max_brightness {
        return Err(AppError {
            message: format!(
                "Brightness {} lm is out of range. Device {} supports {}-{} lm",
                lumens, serial_number, min_brightness, max_brightness
            ),
            error_type: "BrightnessRangeError".to_string(),
        });
    }
    
    handle.set_brightness_in_lumen(lumens).map_err(|e| AppError {
...
pub async fn set_device_brightness_percentage(
    ...
    if percentage > 100 {
        return Err(AppError {
            message: format!("Percentage {} is invalid. Must be between 0-100", percentage),
            error_type: "BrightnessRangeError".to_string(),
        });
    }
...
pub async fn increase_device_brightness(
    ...
    if new_brightness > max_brightness {
        return Err(AppError {
            message: format!(
                "Brightness increase would exceed maximum. Current: {} lm, Increase: {} lm, Max: {} lm",
                current_lumens, lumens_increase, max_brightness
            ),
            error_type: "BrightnessRangeError".to_string(),
        });
    }
    
    handle.set_brightness_in_lumen(new_brightness).map_err(|e| AppError {
...
pub async fn decrease_device_brightness(
    ...
    if new_brightness < min_brightness {
        return Err(AppError {
            message: format!(
                "Brightness decrease would fall below minimum. Current: {} lm, Decrease: {} lm, Min: {} lm",
                current_lumens, lumens_decrease, min_brightness
            ),
            error_type: "BrightnessRangeError".to_string(),
        });
    }
...
pub async fn increase_device_brightness_percentage(
    ...
    if new_percentage > 100 {
        return Err(AppError {
            message: format!(
                "Percentage increase would exceed 100%. Current: {}%, Increase: {}%",
                current_percentage, percentage_increase
            ),
            error_type: "BrightnessRangeError".to_string(),
        });
    }
    ...
    handle.set_brightness_in_lumen(new_lumens).map_err(|e| AppError {
        message: format!("Failed to increase brightness for device {}: {}", serial_number, e),
        error_type: "BrightnessControlError".to_string(),
    })?;
